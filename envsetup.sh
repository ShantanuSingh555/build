...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^cerberus_") ; then
        CERBERUS_BUILD=$(echo -n $1 | sed -e 's/^cerberus_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $CERBERUS_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        CERBERUS_BUILD=
    fi
    export CERBERUS_BUILD
...
function print_lunch_menu()
{
    ...
    tput setaf 1;
    tput bold;
echo " (  ____ \(  ____ \(  ____ )(  ___ \ (  ____ \(  ____ )|\     /|(  ____ \"
echo " | (    \/| (    \/| (    )|| (   ) )| (    \/| (    )|| )   ( || (    \/"
echo " | |      | (__    | (____)|| (__/ / | (__    | (____)|| |   | || (_____ "
echo " | |      |  __)   |     __)|  __ (  |  __)   |     __)| |   | |(_____  )"
echo " | |      | (      | (\ (   | (  \ \ | (      | (\ (   | |   | |      ) |"
echo " | (____/\| (____/\| ) \ \__| )___) )| (____/\| ) \ \__| (___) |/\____) |"
echo " (_______/(_______/|/   \__/|/ \___/ (_______/|/   \__/(_______)\_______)"
    ...
    if [ "z${CERBERUS_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    target=$1
    local variant=$2
    CERBERUS_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/cerberus/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the CERBERUS model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch cerberus_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the cerberus GitHub
        T=$(gettop)
        cd $T > /dev/null
        if [[ $NO_ROOMSERVICE == true ]]; then
            echo "Roomservice turned off, type in 'export NO_ROOMSERVICE=false' if you want it back on"
        else
            vendor/cerberus/build/tools/roomservice.py $product
        fi
        cd - > /dev/null
        check_product $product
    fi
...
function repopick() {
    T=$(gettop)
    $T/vendor/cerberus/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $CERBERUS_FIXUP_COMMON_OUT ]; then
...
function installboot()
{
    ...
    if (adb shell getprop ro.cerberus.device | grep -q "$CERBERUS_BUILD");
    then
        ...
        echo "The connected device does not appear to be $CERBERUS_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.cerberus.device | grep -q "$CERBERUS_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
. $ANDROID_BUILD_TOP/vendor/cerberus/build/envsetup.sh
